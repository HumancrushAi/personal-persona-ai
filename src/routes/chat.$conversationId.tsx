...
function ChatPage() {
  ...
  async function handleSend(e: React.FormEvent) {
    ...
    if (total <= 0) { toast.error("You're out of credits"); navigate({ to: "/credits" }); return; }
    setSending(true);
    ...
    try {
      ...
      if (msg.includes("OUT_OF_CREDITS")) { toast.error("Out of credits"); navigate({ to: "/credits" }); }
  ...
  async function handleSelfie() {
    if (mediaBusy) return;
    const prompt = window.prompt(`What should ${p?.nickname ?? "she"} send a pic of? (optional)`, "") ?? undefined;
    setMediaBusy("selfie");
    try {
      ...
      const msg = err?.message ?? "Error";
      if (msg.includes("OUT_OF_CREDITS")) { toast.error("Not enough credits — selfies cost 8"); navigate({ to: "/credits" }); }
      else toast.error(msg);
  ...
  async function handleVoice() {
    if (mediaBusy) return;
    const last = [...(messages ?? [])].reverse().find(m => m.role === "assistant" && m.kind !== "voice");
    if (!last?.content) { toast.error("Need her to say something first"); return; }
    setMediaBusy("voice");
    try {
      ...
      const msg = err?.message ?? "Error";
      if (msg.includes("OUT_OF_CREDITS")) { toast.error("Not enough credits — voice notes cost 3"); navigate({ to: "/credits" }); }
      else toast.error(msg);
    } finally { setMediaBusy(null); }
  }
  ...
  return (
    <div className="flex h-screen flex-col">
      ...
      <div className="h-1 w-full bg-white/5">
        <div className="h-full bg-grad-primary transition-all" style={{ width: `${(xpInLevel / 15) * 100}%` }} />
      </div>
        ...
        <div className="mx-auto max-w-2xl space-y-3">
          {showOpener && (
            <div className="flex justify-start">
              <div className="max-w-[80%] rounded-2xl rounded-bl-md border border-white/10 bg-white/5 px-4 py-2.5 text-sm">
                <div className="mb-1 text-[10px] uppercase tracking-wide text-primary">{scenario!.emoji} {scenario!.title}</div>
                {scenario!.opener}
              </div>
            </div>
          )}
          ...
          {messages?.map(m => (
            <div key={m.id} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[80%] overflow-hidden rounded-2xl text-sm ${
                m.role === "user"
          ...
          {(sending || mediaBusy) && (
            <div className="flex justify-start">
              <div className="rounded-2xl rounded-bl-md border border-white/10 bg-white/5 px-4 py-2.5 text-sm text-muted-foreground">
                {mediaBusy === "selfie" ? "taking a pic for you…" : mediaBusy === "voice" ? "recording…" : "typing…"}
              </div>
            </div>
          )}
        </div>
      ...
      <form onSubmit={handleSend} className="glass px-3 py-3">
        <div className="mx-auto flex max-w-2xl items-center gap-2">
          <Button type="button" size="icon" variant="ghost"
            onClick={handleSelfie} disabled={!!mediaBusy || sending}
            className="rounded-full" title="Ask for a selfie (8 credits)">
            <ImageIcon className="h-5 w-5 text-primary" />
          </Button>
          <Button type="button" size="icon" variant="ghost"
            onClick={handleVoice} disabled={!!mediaBusy || sending}
            className="rounded-full" title="Get her voice note of last reply (3 credits)">
        ...
        <div className="mx-auto mt-1.5 flex max-w-2xl items-center justify-center gap-3 text-[10px] text-muted-foreground">
          <span className="inline-flex items-center gap-1"><Sparkles className="h-3 w-3 text-primary" />Selfie 8 · Voice 3 · Text 1 credit</span>
        </div>
      </form>
    </div>
  );
