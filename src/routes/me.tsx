...
function MePage() {
  ...
  const { data: conversations } = useQuery({
    enabled: !!userId, queryKey: ["my-convs", userId],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("conversations")
        .select("id, title, updated_at, relationship_level, user_personalities(nickname, companions(image_url))")
    ...
    <div className="min-h-screen">
      ...
      <header className="mx-auto flex max-w-4xl items-center justify-between px-6 py-5">
        ...
        <div className="flex items-center gap-2">
          <Link to="/credits" className="inline-flex items-center gap-1.5 rounded-full bg-white/5 px-3 py-1.5 text-xs font-medium ring-1 ring-white/10">
            <Coins className="h-3.5 w-3.5 text-primary" /> {(balance?.free_messages_remaining ?? 0) + (balance?.paid_credits ?? 0)}
          </Link>
      ...
      <section className="mx-auto max-w-4xl px-6 pb-20">
        <div className="flex items-center justify-between">
          ...
          <Button asChild className="rounded-full bg-grad-primary text-primary-foreground shadow-glow">
            <Link to="/browse"><Plus className="mr-1 h-4 w-4" /> New</Link>
          </Button>
        </div>
              ...
              <Link key={c.id} to="/chat/$conversationId" params={{ conversationId: c.id }}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 text-xs text-muted-foreground">
                    <Heart className="h-3 w-3 fill-primary text-primary" /> Level {c.relationship_level ?? 1}
                    <span>·</span>
